...
__all__ = ["ResnetUnit", "ResnetStage"]
# Deep Residual Learning for Image Recognition (https://arxiv.org/abs/1512.03385)
# 2015

class ResnetUnit(BottleNeck):
    def __init__(self, in_channels, out_channels, internal_expansion: float = 0.25, kernel_size: Union[int, Tuple[int, int]] = 3,
                 stride: int = 1, dilation: int = 1, drop_path_rate: float = 0., use_nonlinear: bool = True):
        super(ResnetUnit, self).__init__(in_channels, out_channels, internal_expansion, kernel_size, stride, dilation, drop_path_rate, use_nonlinear)
...
class ResnetStage(nn.Module):
    def __init__(self, in_channels: int, out_channels: int, num_units: int, kernel_size: Union[float, Tuple[float, float]]  = (3, 3), 
                 stride: int = 2, dilation: int = 1, use_nonlinear: bool = True) :
        """ Build a stage of Resnet model.
        Args:
        ...
        units = [ResnetUnit(in_channels, out_channels, internal_expansion=1/stride, kernel_size=kernel_size, 
                                     stride=stride, dilation=dilation, use_nonlinear=use_nonlinear)]
        for i in range(num_units-1):
            units.append(ResnetUnit(out_channels, out_channels, internal_expansion=0.25, kernel_size=kernel_size, 
                                     stride=1, dilation=dilation, use_nonlinear=use_nonlinear))

        self.resnet_stage = nn.Sequential(*units)

    def forward(self, x):
        return self.resnet_stage(x)
...
if __name__ == '__main__':
    ...
    for i, opt in enumerate(stage_config):
        ...
        resnext = ResnetStage(input.size(1), opt[0], num_units=opt[1], stride=opt[2])
        resnext.to(device)
        resnext.eval()
        ...
        save_model_to_onnx(resnext.to("cpu"), input.to("cpu"), f"resnet_stage{i}.onnx") 
        input = out
